...
// ---------------------------------------------------------------------------
// git_diff — local git subprocess with GitHub API fallback
// ---------------------------------------------------------------------------

const DIFF_TRUNCATE_LIMIT: usize = 100_000;

/// Try local `git diff` first; fall back to GitHub compare API if refs are
/// missing (common in shallow clones).
#[pyfunction]
#[pyo3(signature = (
    base,
    head,
...
pub fn git_diff(
    ...
    match git_diff_local(base, head, repo_path, stat_only, path_filter, context) {
        Ok(text) => Ok(text),
        Err(local_err) => {
            // Resolve repo for API fallback
            let repo_str = match repo {
                Some(r) => Some(r.to_string()),
                None => detect_git_repo(repo_path),
            };
            if let Some(ref r) = repo_str {
                match git_diff_github_api(r, base, head, stat_only, path_filter) {
                    Ok(text) => Ok(text),
                    Err(api_err) => Ok(format!(
                        "git diff failed locally and via GitHub API.\n\
                         Local: {}\nAPI: {}",
                        local_err, api_err
                    )),
                }
...
fn git_diff_local(
    ...
    let output = cmd
        .output()
        .map_err(|e| format!("Failed to run git diff: {}. Is git installed?", e))?;
...
fn git_diff_github_api(
    ...
    let data = gh_get(&endpoint)?;

    let glob_matcher = path_filter.and_then(|pat| {
    ...
    let files: Vec<&Value> = files
        ...
        .filter(|f| {
            if let Some(ref gs) = glob_matcher {
                let fname = f.get("filename").and_then(|v| v.as_str()).unwrap_or("");
                gs.is_match(fname)
            } else {
                true
            }
    ...
    if files.is_empty() {
        return Ok(format!(
            "No differences between {} and {} (via GitHub API).",
            base, head
        ));
    }
    ...
    let total_commits = data
        ...
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    let ahead_by = data.get("ahead_by").and_then(|v| v.as_u64()).unwrap_or(0);
    let commit_count = if ahead_by > 0 {
        ahead_by
    } else {
        total_commits
    };
    let mut out = format!(
        "Comparing {}...{} ({} commit{}, via GitHub API)\n\n",
        base,
        head,
        commit_count,
    ...
    if stat_only {
        // Stat-only: file list with +/- counts
        let mut total_add: u64 = 0;
        let mut total_del: u64 = 0;
        for f in &files {
            let fname = json_str(f, "filename");
            let status = json_str(f, "status");
        ...
        // Full diff: reconstruct from patch fields
        for f in &files {
            let fname = json_str(f, "filename");
            let status = json_str(f, "status");
            ...
            match status.as_str() {
                "added" => out.push_str("new file\n"),
                "removed" => out.push_str("deleted file\n"),
                "renamed" => {
                    let prev = json_str(f, "previous_filename");
                    if !prev.is_empty() {
...
fn truncate_diff(text: String) -> String {
