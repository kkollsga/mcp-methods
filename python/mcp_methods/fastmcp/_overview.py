...
The graph object passed in must expose `describe(types=None, connections=False,
limit=20) -> str` — kglite's `KnowledgeGraph` has this method. The
...
def register_overview(app, graph, *, overview_prefix: str | None = None) -> None:
    ...
    `graph` is any object exposing `describe(types, connections, limit)`.
    """
    ...
    @app.tool(
        description=(
            ...
            "details. `limit` caps per-type sample size (default 20)."
        )
    )
    def graph_overview(
        ...
        body = graph.describe(types=types, connections=connections, limit=limit)
