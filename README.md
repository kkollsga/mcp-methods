...
| `SkillRegistry`, `Skill`, `register_skills_as_prompts` | Operator-authored methodology as MCP prompts (layered: project → domain pack → manifest-inline → owned → bundled defaults). See `crates/mcp-methods/dev-documentation/skills-aware-mcp.md` for the design doc. |
